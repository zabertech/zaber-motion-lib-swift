...
public final class Pvt: @unchecked Sendable {
    ...
    /**
     ...
     - Parameters:
        - pvtBufferNumber: The ID number of the PVT buffer to control. PVT buffer numbers start at one.

     - Returns: PvtBuffer instance.
     */
    public func getBuffer(pvtBufferNumber: Int) throws -> PvtBuffer {
        guard pvtBufferNumber > 0 else {
            throw ZaberMotionExceptions.InvalidArgumentException(message: "Invalid value; PVT buffers are numbered from 1.")
        }

        return PvtBuffer(device: self.device, bufferNumber: pvtBufferNumber)
    }
